...
class triple {
  ...
  function set($subject,$predicate,$object,$allow_multiple=false) {
    global $db;
    write_to_log("setting $predicate of $subject to $object");
    // get id's
    $data = array(
    ...
    if($allow_multiple) { // allow setting multiple objects for one subject and predicate
      if(self::exists($data['subject_id'],$data['predicate_id'],$data['object_id'])) {
        $db->update('triples',$data,array(
          'subject_id' => $data['subject_id'],
          'predicate_id' => $data['predicate_id']
        ));
        debug_dump("updating ".json_encode($data));
      } else {
        debug_dump("inserting ".json_encode($data));
        $db->insert('triples',$data);
      }
    } else { // allow only one object for a given subject and predicate
